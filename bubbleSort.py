...
def bubble_sort(my_list: list):
    total_swaps = 0
    list_to_sort_length = len(my_list)
    swapped = True

    print("BUBBLE START: Sorting the list", my_list)
    ...
    while swapped and list_to_sort_length >= 0:
        ...
        for i in range(0, list_to_sort_length - 1):
            ...
            if my_list[i] > my_list[i + 1]:
                print("\t\t\t" + str(my_list[i]) + " is greater than " + str(my_list[i + 1]) + " so swapping")
                print("\t\t\t" + str(my_list))
                ...
                unswapped_lower = str(my_list[0:i] if len(my_list[0:i]) > 0 else "") + " "
                swapped = "\033[1m" + str(my_list[i + 1]) + " <-> " + str(my_list[i]) + ", " + "\033[0m"
                unswapped_higher = str(my_list[i + 2:] if len(my_list[i + 2:]) > 0 else "") + " "
        ...
        print("\tWHILE END: Swapped is " + str(swapped) + ", list_to_sort_length is " + str(
            list_to_sort_length) + ", and list is", my_list)

    print("BUBBLE END: Sorted list is ", my_list)
